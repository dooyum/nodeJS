...
angular.module('myApp.controllers', ['nvd3ChartDirectives'])
  .controller('AppCtrl', function ($scope, $http, socket ) {
    ...
    socket.on('cpu:data', function (data) {
        $scope.transcript1 = $scope.transcript1 + " " + data.word;
    });
    ...
    $scope.modifyCount = function(device, count){
        var workData;
        if(device == 'cpu')
            workData = $scope.exampleData2;
        else
            workData = $scope.exampleData;

        var valueStored = workData[0].values;

        for(var i = 0; i < valueStored.length - 1; i++){
            valueStored[i][0] = valueStored[i + 1][0];
    ...
    $scope.transcript1 = "This is a really long box to be displayed";
    $scope.transcript2 = "";

    $scope.transcript1Count = 0;
    $scope.transcript2Count = 0;

    $scope.duplicateTranscript = function(){
     // $scope.transcript1 = $scope.transcript1 + $scope.transcript1;
      
      var sample = $scope.sample;
      var i = $scope.transcript2Count;
      var end = i + 5;
      for(i; i < end; i++){
        $scope.transcript2 = $scope.transcript2 + " " + sample[i].word;
      }

      $scope.transcript2Count += 5;
      $scope.addToTranscript($scope.exampleData2, $scope.transcript2Count);
    }
    ...
    */
    ...
    $scope.addToTranscript = function(data, count){
      var valueStored = data[0].values;

      console.log();

     // .values[][count]
     // $scope.exampleData[0].values[x][1]
      //for(var i = 0; i < values.length - 1; i++){
      
      //for(var x in $scope.exampleData[0].values){ 
      ...
      for(var i = 0; i < valueStored.length - 1; i++){
        ...
        values[i] = new Array();
        values[i][0] = valueStored[i + 1][0];
        values[i][1] = valueStored[i + 1][1];

        //values[0] = $scope.exampleData[0].values[x][0] + 20;
        //values[1] = $scope.exampleData[0].values[xInt.toString()][1];
        array.push(values);
      }
      */

      valueStored[valueStored.length - 1][0] = valueStored[valueStored.length - 1][0] + 1;
      valueStored[valueStored.length - 1][1] = valueStored[valueStored.length - 1][1] + 1;
      

      $scope.exampleData = [{ "key": "Series 1", "values": valueStored}]; 
    ...
    */
    ...
    $scope.exampleData = [{
      "key": "GPU",
      "values": [ [0, 0] , [1,15], [2,25], [3,15], [4,20] ]
    }];
    

    $scope.modifyValues = function(){
        ...
        for(var x in $scope.exampleData[0].values){ 
          ...
          values[0] = $scope.exampleData[0].values[x][0] + 20;
            ...
            if(x < 4)
              ...
              values[1] = $scope.exampleData[0].values[x][1] + Math.floor((Math.random()*10)+1);
            values["series"] = 0;
            array.push(values);
          }

          //this is the variable that needs to be updated, the view will update automatically
          $scope.exampleData = [{ "key": "Series 1", "values": array}]; 

          var array2 = new Array();
          for(var y in $scope.exampleData2[0].values){  
            var values2 = new Array();
            values2[0] = $scope.exampleData2[0].values[y][0] + 20;

            var yInt = parseInt(y);
            yInt++;

            if(y < 4)
              values2[1] = $scope.exampleData2[0].values[yInt.toString()][1];
            else
              values2[1] = $scope.exampleData2[0].values[y][1] +  Math.floor((Math.random()* 1)+1);
          ...
          //this is the variable that needs to be updated, the view will update automatically
          $scope.exampleData2 = [{ "key": "Series 1", "values": array2}]; 
          $scope.$apply();
      },1000);
    }
